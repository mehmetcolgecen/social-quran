...
// Ses kimliği → okunacak metin. Aynı kimlik farklı metinle iki kez tanımlanırsa hata.
const items = new Map<string, string>();
function add(id: string | undefined, text: string) {
  if (!id) return;
  const prev = items.get(id);
  if (prev !== undefined && prev !== text) {
    throw new Error(`Ses kimliği çakışması: "${id}" hem "${prev}" hem "${text}" için kullanılmış`);
  }
  items.set(id, text);
}
...
async function synth(text: string, rate: string): Promise<Buffer> {
  if (!tts) {
    tts = new MsEdgeTTS();
...
for (const [id, text] of items) {
  ...
  for (let deneme = 1; deneme <= 3 && !buf; deneme++) {
    try {
      buf = await synth(text, rateFor(id));
    } catch (e) {
      tts = null; // bağlantı koptuysa yeniden kur
...
console.log(`\nBitti: ${yeni} üretildi, ${atlanan} zaten vardı, ${hata} hata, toplam ${items.size} öğe (ses: ${VOICE}).`);
